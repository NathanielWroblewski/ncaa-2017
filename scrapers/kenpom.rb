...
CSV.open('./datasets/teams.csv', 'w') do |csv|
  csv << %w(team offense defense year)
  ...
  YEARS.each do |year|
    ...
    rows = teams.zip(adjusted_offense, adjusted_defense)
    ...
    rows.each do |row|
      ...
      csv << [team_name, offense, defense, year.to_i]
    end
